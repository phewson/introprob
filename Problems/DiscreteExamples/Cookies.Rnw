...
\begin{displaymath}
P(X=\Sexpr{question}) = \frac{\lambda^x e^{-\lambda}}{x!} = \frac{\Sexpr{round(chocchips/cookiebatch,3)}^{\Sexpr{question}} e^{-\Sexpr{round(chocchips/cookiebatch,3)}}}{\Sexpr{question}!} = \Sexpr{round(dpois(question, chocchips/cookiebatch),3)}
\end{displaymath}
