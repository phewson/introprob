...
Secondly, you need to recognise that in order to find $P(X>\Sexpr{k})$ you are going to have to find the probability of the complement of the event $X>\Sexpr{k}$ (which is $X \leq \Sexpr{k}$) and then use
$1-P(X \leq \Sexpr{k})$
...
\begin{displaymath}
P(X > \Sexpr{x}) = 1-P(T \leq \Sexpr{x}) = 1 - \Sexpr{round(companswer,4)} = \Sexpr{round(answer,4)}
\end{displaymath}
