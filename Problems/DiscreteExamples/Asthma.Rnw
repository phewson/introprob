...
Unlike the dice example last week, here you are asked to directly find $P(X \leq \Sexpr{visits})$ i.e. $F(\Sexpr{visits}) = f(0) + \cdots + f(\Sexpr{visits})$

You should have developed a nice recursion for doing this by now, so

\begin{displaymath}
P(X \leq \Sexpr{visits}) = \Sexpr{prob} + (1-\Sexpr{prob}) f(0) + \cdots + (1-\Sexpr{prob})f(\Sexpr{visits-1})
\end{displaymath}
