...
\begin{displaymath}
P(X \leq x) = \int_{0}^x \theta e^{-\theta t} dt = 1-e^{-\theta x}
\end{displaymath}
so you just need to solve this for $x$.

The next question (which you can answer in a computer) you are to assume that $X \sim \mbox{Exp}(\theta)$ where $\theta = \Sexpr{theta}$.   If $p$ = \Sexpr{p} then you should find $x$ such that
