...
\begin{displaymath}
P(A_i | B) = \frac{ P(B|A_i)P(A_i)}{\sum_{j=1}^3 P(B|A_i)P(A_i)}
\end{displaymath}

\begin{itemize}
\item We need to note that $P(Pass)$ is denoted $P(B)$
\item We need to note that the $P(Pass | Instruction = i)$ is denoted $P(B | A_i)$ for instructors $i=1,2,3$
\end{itemize}

The probabilities $P(Pass | Instruction = i)$ are given in the question.   There were \Sexpr{BD} students in Bonzo's class, \Sexpr{BO} of these students passed so $P(Pass | Instruction = 1) = \frac{\Sexpr{BO}}{\Sexpr{BD}} = \Sexpr{round(BO/BD,3)}$.
