...
Assume he picks either of the three routes with equal probability.   Further assume each time he has a fall he gets amnesia and forgets how he tried to get out last time and is put back in his cell immediately.
...
\begin{solution}
The exit routes Duct, Sewer, Chimney, Door.   $X$ is the time taken to exit, and the possible outcomes are $x_1 = \Sexpr{A}, x_2=\Sexpr{B}, x_3=\Sexpr{C}, x_4=0)$.

Because we are still working with naive probability we assume that $P(X=x) = \frac{1}{4}$ for each route. 

Therefore, the expectation of time taken is $E[X] = \sum_{i=1}^4 x_i \times P(X=x_i)$ which is:
